...
function convertirLongitudYAlert(valor, unidadOrigen, unidadDestino) {
	...
	if (
		!factoresConversion[unidadOrigen] ||
		!factoresConversion[unidadDestino]
	) {
	...
	alert(
		`${valor} ${unidadOrigen} es igual a ${resultado.toFixed(
			2
		)} ${unidadDestino}`
	);
}
...
const unidadOrigen = prompt(
	'Seleccione la unidad de origen (km, hm, dam, m, dm, cm, mm):'
);
const unidadDestino = prompt(
	'Seleccione la unidad de destino (km, hm, dam, m, dm, cm, mm):'
);
